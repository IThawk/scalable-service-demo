...
public class SimpleSplitJdbcTemplate extends SplitJdbcTemplate implements SimpleSplitJdbcOperations {
	...
	protected <K, T> List<T> doSearch(K splitKey, final T bean, String name, Object valueFrom, Object valueTo, SearchOper searchOper) {
		...
		log.info("SimpleSplitJdbcTemplate.doSearch, splitKey={} dbPrefix={} tablePrefix={} dbNo={} tableNo={}.", splitKey, dbPrefix, tablePrefix, dbNo, tableNo);

		SplitDb sn = splitDbs.get(dbNo);
		JdbcTemplate jt = getReadJdbcTemplate(sn);

		SqlUtil.SqlRunningBean srb = null;
	...
	protected <K, T> T doSelect(K splitKey, final Class<T> clazz, String name, Object value) {
		...
		log.info("SimpleSplitJdbcTemplate.doSelect, splitKey={} dbPrefix={} tablePrefix={} dbNo={} tableNo={}.", splitKey, dbPrefix, tablePrefix, dbNo, tableNo);

		SplitDb sn = splitDbs.get(dbNo);
		JdbcTemplate jt = getReadJdbcTemplate(sn);

		SqlUtil.SqlRunningBean srb = SqlUtil.generateSelectSql(name, value, clazz, dbPrefix, tablePrefix, dbNo, tableNo);
	...
	protected <K, T> void doUpdate(K splitKey, final Class<?> clazz, UpdateOper updateOper, T bean, long id) {
		...
		log.info("SimpleSplitJdbcTemplate.doUpdate, splitKey={} dbPrefix={} tablePrefix={} dbNo={} tableNo={}.", splitKey, dbPrefix, tablePrefix, dbNo, tableNo);
