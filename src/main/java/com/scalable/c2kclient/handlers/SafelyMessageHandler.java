...
import com.scalable.c2kclient.excephandler.DefaultExceptionHandler;
...
public abstract class SafelyMessageHandler implements MessageHandler {
		...
		for (ExceptionHandler excepHandler : excepHandlers) {
			...
			if (excepHandler.support(t)) {
				try {
					...
					log.error("Exception hanppens when the handler {} is handling the exception {} and the message {}. Please check if the exception handler is configured properly.", excepHandler.getClass(), t.getClass(), message);
					log.error("The stack of the new exception on exception is, ", e);
				}
			}
		}
	...
	protected abstract void doExecute(String message) throws Exception;
